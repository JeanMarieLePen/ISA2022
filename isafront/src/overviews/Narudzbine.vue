            ...
            <table id="tbl_order" style="margin-left:30px;" class="col-md-3" v-for="(temp, index) in narudzbine" :key="index">
                ...
                <tr>
                    <td colspan="3" style="text-align:center;">
                        <button @click="delivery(temp.id)" style="margin-top:5px;margin-bottom:5px;">Isporuka</button>
                    </td>
                </tr>
            </table>
        </div>
...
export default {
    ...
    methods:{
        delivery(idPorudzbine){
            this.$router.push(`/transport/${idPorudzbine}`);
        },
        formDate(datum){
        ...
        getAllPorudzbineByWorkersId(){
            dataService.getAllPorudzbineByWorkersId(this.userId).then(response => {
                console.log("DOBAVLJENE PORUDZBINE: " + response.data.length);
    ...
    created(){
        if (localStorage.getItem("token").toString() !== "null") {
            ...
            try {
                ...
                this.getAllPorudzbineByWorkersId();
            } catch (error) {
